...
# Statistical summary for all breast-milk contrasts
# - Load DELTA_framework/delta_table.csv for each comparison under results/results_milk
...
base_dir <- file.path("results", "results_milk")
emp_fdr_cutoff <- 0.2
bh_cutoff <- 0.1
plot_dir <- file.path("results", "other_plots", "locfdr_milk")
...
comparisons <- sort(basename(delta_dirs))
...
results <- lapply(comparisons, run_one)
...
save_locfdr_grid <- function(plot_paths, titles, out_path, ncol = 2) {
  n <- length(plot_paths)
...
cat("\n\n=== Summary across all results_milk comparisons ===\n")
...
if (nrow(empirical_hits_all) > 0) {
  print(
    ...
    n = min(300, nrow(empirical_hits_all))
  )
} else {
  cat("None\n")
}
...
if (interactive()) {
  if (nrow(empirical_hits_all) > 0) {
    View(empirical_hits_all %>%
           filter(comparison != "mom_milk_T7_preterm_yes_vs_mom_milk_T7_preterm_no", p_adj_bh < 0.1) %>%
